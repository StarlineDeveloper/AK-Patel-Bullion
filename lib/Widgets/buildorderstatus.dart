...
class _OrderStatus_WidgetState extends State<OrderStatus_Widget> {
        ...
        children: [
          ...
          Row(
            children: [
              ...
              CustomText(
                text: widget.dataList.source!.toLowerCase().trim() == "gold" ||
                        widget.dataList.source!.toLowerCase().trim() ==
                            "goldnext"
                    ? '(${widget.dataList.volume!} gm) : ${widget.dataList.rate} -> ${widget.dataList.total}'
                    : widget.dataList.source!.toLowerCase().trim() ==
                                "silver" ||
                            widget.dataList.source!.toLowerCase().trim() ==
                                "silvernext"
                        ? '(${widget.dataList.volume!} kg) : ${widget.dataList.rate} -> ${widget.dataList.total}'
                        : 'Unknown',
          ...
          CustomText(
            text: widget.dataList.openTradeDateTime!,
            textColor: AppColors.textColor,
            size: 14.0,
            fontWeight: FontWeight.normal,
          ),
  ...
  void openEditLimitPopup(
    ...
    setState(() {
      _priceController.text = rate.toString();
    });
    showDialog(
    ...
    Functions.checkConnectivity().then((isConnected) {
      if (isConnected) {
        ...
        var objVariable = updateLimitReqToJson(UpdateLimitRequest(
          ...
          volume: widget.dataList.volume!.toString(),
